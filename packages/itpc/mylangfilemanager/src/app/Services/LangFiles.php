...
namespace ITPC\MyLangFileManager\app\Services;
...
class LangFiles
{
    ...
    public function getlangFiles()
    {
        ...
        foreach (is_dir($this->getLangPath()) ? scandir($this->getLangPath(), SCANDIR_SORT_DESCENDING) : [] as $file) {
            ...
            if (! in_array($fileName, array_merge(['.', '..'], config('itpc.mylangfilemanager.language_ignore')))) {
                $fileList[] = [
                    'name' => ucfirst(str_replace('_', ' ', $fileName)),
                    'original_name' => $fileName,
                    'url' => url(config('itpc.base.route_prefix', 'admin')."/my-language/texts/{$this->lang}/{$fileName}"),
    ...
    public function displayInputs($fileArray, $parents = [], $parent = '', $level = 0)
    {
        ...
        foreach ($fileArray as $key => $item) {
            if (is_array($item)) {
                echo view()->make('mylangfilemanager::language_headers', ['header' => $key, 'parents' => $parents, 'level' => $level, 'item' => $item, 'langfile' => $this, 'lang_file_name' => $this->file])->render();
            } else {
                echo view()->make('mylangfilemanager::language_inputs', ['key' => $key, 'item' => $item, 'parents' => $parents, 'lang_file_name' => $this->file])->render();
            }
        }
    }
